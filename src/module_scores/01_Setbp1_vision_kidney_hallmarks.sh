#!/bin/bash
## run the Rscript 14_Setbp1_vision_kidney.R and schedule this job to SLURM
...
singularity exec --cleanenv --no-home -B ${wd} ${wd}/bin/docker/setbp1_manuscript_1.0.11.sif Rscript --vanilla ${src}/14_Setbp1_vision_kidney.R # here vanilla ensures only the script is run and environment is kept clean