...
Now that module scores have been calculated, we wanted to see if any were statistically significant for any cell types between conditions. 
# ```{r}
...
#   het_apop[[i]] <- as.data.frame(split_het[[i]]@meta.data[["aki_up_score1"]])
#   het_apop[[i]]$condition <- "heterozygous"
#   het_apop[[i]][["cell_type"]] <- i
#   colnames(het_apop[[i]]) <- c("aki_up_score", "condition", "cell_type")
# }
# 
# ctrl_apop <- list()
# for (i in names(split_ctrl)) {
#   ctrl_apop[[i]] <- as.data.frame(split_ctrl[[i]]@meta.data[["aki_up_score1"]])
#   ctrl_apop[[i]]$condition <- "control"
#   ctrl_apop[[i]][["cell_type"]] <- i
#   colnames(ctrl_apop[[i]]) <- c("aki_up_score", "condition", "cell_type")
# }
# 
...
#   summarise(
#     count = n(),
#     median = median(aki_up_score, na.rm = TRUE),
#     IQR = IQR(aki_up_score, na.rm = TRUE)
#   )
# 
# print(summary_df)
# 
# ## visualize
# ggplot(data = combined_df, aes(x = condition, y = aki_up_score)) +
#   geom_boxplot() +
...
#   apop_res_list[[i]] <- wilcox.test(het_data$aki_up_score, ctrl_data$aki_up_score, exact = FALSE)
# }
# 
