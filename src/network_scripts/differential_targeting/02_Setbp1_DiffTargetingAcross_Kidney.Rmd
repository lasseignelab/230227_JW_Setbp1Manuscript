...
This analysis determines differential gene targeting scores across cell types between conditions from the PANDA regulatory networks in order to identify cell-specific gene targeting. All networks below were filtered to only include positive weights. 
...
save(het_enriched, file = here("results/diff_targeting/kidney_het_enriched_gene_within.Rdata"))
```
...
save(ctrl_enriched, file = here("results/diff_targeting/kidney_ctrl_enriched_gene_within.Rdata"))
```
