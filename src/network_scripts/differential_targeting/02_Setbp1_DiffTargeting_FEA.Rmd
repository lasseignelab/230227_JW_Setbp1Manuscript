...
# all cortex cell types
genes <- as.character(data$gene)
fea_result_filt_all <- fea(
  genes = genes,
  organism = "mmusculus",
  sources = c("GO:BP")
)
...
data <- cortex_fea[, c("term_name", "source", "intersection_size")]
...
annotation_colors <- list(`Cell Type` = c(
  ...
  "microglia" = "#C216B7"
)) # "oligodendrocytes" = "#7997FF" (none for pathways)
...
cor_heatmap <- Heatmap(mat,
  col = colorRampPalette(c("black", "yellow"))(4),
  heatmap_legend_param = list(title = "Intersection Size"),
  cluster_rows = TRUE,
  ...
  column_title = "Cell Type", row_title = "Pathways", row_title_side = "right",
...
png(here("results/diff_targeting/cortex_het_enriched_DT_pathways.png"),
  res = 250,
  height = 5000,
  width = 4000
)
...
FEA of het enriched DT genes in kidney
```{r}
data <- kidney_DT[kidney_DT$gene %in% setbp1_genes, ]
...
for (i in cell_types) {
  temp <- data[data$`Cell Type` == i, ]
  genes <- as.character(temp$gene)
  if (i == "Bcell" | i == "CDIC_typeB" | i == "PCTS1" | i == "PST" | i == "smcs") {
    fea_result_filt <- fea_no_sig(
      genes = genes,
      organism = "mmusculus",
      sources = c("GO:BP", "HP")
    ...
    fea_result_filt <- fea(genes = genes, organism = "mmusculus", sources = c("GO:BP", "HP"))
    fea_celltypes[[i]] <- fea_result_filt
    print(paste0("fea analysis complete on ", i))
  }
}
...
data <- kidney_fea[, c("term_name", "source", "intersection_size")]
...
png(here("results/diff_targeting/kidney_het_enriched_DT_pathways.png"),
  res = 250,
  height = 5000,
  width = 3000
)
