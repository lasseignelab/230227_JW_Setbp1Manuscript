...
```{r}
#writing function for below data wrangling 
files
```
## Visualizing edge weight distributions filtered and unfiltered
...
Bcell.melt <- as.data.frame(melt(Bcell.control.regNet, varnames = c("TF", "gene"), value.name = "ctrl_edge_weight"))#melting dataframe
Bcell.melt <- as.data.frame(Bcell.melt[,3])
colnames(Bcell.melt) <- "edge_weight"
Bcell.melt$cell_type <- "Bcell"
...
png(filename = here("results/diff_targeting/cortex_regNet_edgeweight_filtered_dist_ctrl.png"),
    width = 1000,
    height = 500)
ggplot(data=merged_control_post, aes(x=edge_weight, group=cell_type, color=cell_type)) +
...
```{r calc-targeting}
for (i in control_networks){
  targetingCalc(regNetmatrix = paste0(i, ".regNet"), variable_name = sub("_control.*", "", i), edge_weight_name = ctrl_edge_weight, condition = "ctrl")
}
