...
png(
  filename = here("results/diff_targeting/kidney_gene_targeting_dist_het.png"),
  width = 1000,
  height = 500
)
...
targeting_heatmap(annotation_colors, data, meta_colname, plot_path, rowtitle, plot_title, show_names = TRUE, width = 1000, height = 2000, show_names = TRUE)
```
...
png(
  filename = here("results/diff_targeting/kidney_TF_targeting_dist_het.png"),
  width = 1000,
  height = 500
)
ggplot(data = pivoted, aes(x = targeting_score, group = cell_type, color = cell_type)) +
...
rowtitle <- "TFs"
# setting columntitle
plot_title <- "Targeting of top 100 TF Across Cell-types in Setbp1 S858R Kidney Cortex"
...
png(
  filename = here("results/diff_targeting/kidney_gene_targeting_dist_ctrl.png"),
  width = 1000,
  height = 500
)
ggplot(data = pivoted, aes(x = targeting_score, group = cell_type, color = cell_type)) +
...
png(
  filename = here("results/diff_targeting/kidney_TF_targeting_dist_ctrl.png"),
  width = 1000,
  height = 500
)
ggplot(data = pivoted, aes(x = targeting_score, group = cell_type, color = cell_type)) +
...
targeting_heatmap(annotation_colors, data, meta_colname, plot_path, rowtitle, plot_title, show_names = TRUE, show_names = TRUE, height = 2200)
```
...
nonzero <- pivoted[pivoted$targeting_score != 0, ]
png(
  filename = here("results/diff_targeting/kidney_difftargeting_het_TF_distribution.png"),
  width = 1000,
  height = 500
)
ggplot(data = nonzero, aes(x = targeting_score, group = cell_type, color = cell_type)) +
...
nonzero <- pivoted[pivoted$targeting_score != 0, ]
png(
  filename = here("results/diff_targeting/kidney_difftargeting_ctrl_TF_distribution.png"),
  width = 1000,
  height = 500
)
ggplot(data = nonzero, aes(x = abs(targeting_score), group = cell_type, color = cell_type)) +
...
kidney_het_enriched_top_TFs <- do.call(bind_rows, TF_diff_het) # collapsing list into single df using row binding
...
kidney_het_enriched_all_TFs <- do.call(bind_rows, TF_diff_het) # collapsing list into single df using row binding
```

```{r visualize-het-enriched-TF-top20-heatmap}
## formatting input data
data <- kidney_het_enriched_top_TFs[, c("TF", "targeting_score", "cell_type"), drop = FALSE]
data <- pivot_wider(data, names_from = c("cell_type"), values_from = "targeting_score", names_repair = "minimal")
data[is.na(data)] <- 0
names <- data$TF # add TF names as rownames
data <- data[, -1, drop = FALSE] # drop TF column
...
```{r}
## pathway analysis and plotting
data <- kidney_het_enriched_top_TFs
...
kidney_ctrl_enriched_top_TFs <- do.call(bind_rows, TF_diff_ctrl) # collapsing list into single df using row binding
...
kidney_ctrl_enriched_all_TFs <- do.call(bind_rows, TF_diff_ctrl) # collapsing list into single df using row binding
```

```{r visualize-ctrl-enriched-TF-top20-heatmap}
## formatting input data
data <- kidney_ctrl_enriched_top_TFs[, c("TF", "targeting_score", "cell_type"), drop = FALSE]
data <- pivot_wider(data, names_from = c("cell_type"), values_from = "targeting_score", names_repair = "minimal")
data[is.na(data)] <- 0
names <- data$TF # add TF names as rownames
data <- data[, -1, drop = FALSE] # drop TF column
...
targeting_heatmap(annotation_colors, data, meta_colname, plot_path, rowtitle, plot_title, show_names = TRUE, 5000)
```
...
```{r}
## pathway analysis and plotting
data <- kidney_ctrl_enriched_top_TFs
...
```
  user   system  elapsed 
2505.406  508.738 3237.780

```{r}
# run style
style_file(here("src/network_scripts/differential_targeting/Setbp1_DiffTargetingKidney_01.Rmd"))
