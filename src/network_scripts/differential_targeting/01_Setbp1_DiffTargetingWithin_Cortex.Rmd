...
date: '2023-02-13'
---
note to code reviewer: this was run in docker 1.0.5
...
ggplot(data = merged_control, aes(
  ...
  theme_bw() +
  scale_color_manual(values = c("#6968B4",
                                "#DC71FA",
                                "#0a2f6f",
                                "#00BBDB",
                                "#C216B7",
...
* Using the combined het/ctrl dataframe for each cell type above, calculate the difference (het-ctrl) for the targeting weight where the more positive difference indicates higher gene targeting in het. 
...
#het_enriched <- replace(diff_gene_targeting, diff_gene_targeting[, ] < 0, 0) # creating dataframe where the differential is enriched in the het condition (commented out because of above note)
...
oligodendrocytes<- het_enriched[, c(1,8)]
...
opcs<- het_enriched[, c(1,9)]
opcs<- subset(opcs, opcs <= -49.718 | opcs >= 101.878)
colnames(opcs) <- c("gene", "targeting_score")
opcs$cell_type <- "opcs"

filtered_het <- bind_rows(astrocytes, excitatory_neurons, fibro_cortex, inhibitory_neurons, microglia, pericytes, oligodendrocytes, opcs)
```
...
rownames(data) <- c("astrocytes", "excitatory neurons", "fibroblasts", "inhibitory neurons", "microglia", "pericytes", "oligodendrocytes", "opcs")
