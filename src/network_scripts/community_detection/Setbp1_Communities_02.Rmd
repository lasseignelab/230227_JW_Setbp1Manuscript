...
#mural ----------
mural_het <- as.data.frame(mural.heterozygous_regNet) %>% mutate(TF = rownames(mural.heterozygous_regNet)) %>% pivot_longer(names_to = "gene", values_to = "het_weight", cols = c(1:22033))
mural_het$het_weight <- ifelse(mural_het$het_weight < 0, 0, mural_het$het_weight)

mural_ctrl <- as.data.frame(mural.control_regNet) %>% mutate(TF = rownames(mural.control_regNet)) %>% pivot_longer(names_to = "gene", values_to = "ctrl_weight", cols = c(1:22033))
mural_ctrl$ctrl_weight <- ifelse(mural_ctrl$ctrl_weight < 0, 0, mural_ctrl$ctrl_weight)

mural_all <- merge(mural_ctrl, mural_het, by = c("TF", "gene"))
...
save(mural_all, file = here("data/processed/network_analysis/mural_regNet_pos.Rdata"))
...
load(here("data/processed/network_analysis/mural_regNet_pos.Rdata"))
```
...
```{r mural-alpaca}
mural_alpaca <- alpaca(mural_all, file.stem = here("results/alpaca/mural"), verbose = TRUE)
mural_alpaca_mat <- mural_alpaca[[1]] 
mural_alpaca_memberships <- as.vector(mural_alpaca_mat)
names(mural_alpaca_memberships) <- names(mural_alpaca_mat)
mural_alpaca_memberships
```
...
toevaluate <- c("astro", "ex") #comment in rest later: , "in", "micro", "mural", "oligo", "opcs")
...
toevaluate <- c("astro") #comment in rest later: , "ex", "in", "micro", "mural", "oligo", "opcs")
