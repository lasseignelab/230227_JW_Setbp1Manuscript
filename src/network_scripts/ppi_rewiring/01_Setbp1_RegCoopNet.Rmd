...
Step 8. compare seed (STRING) ppi and ppi after running PANDA
...
seed_ppi <- read.table(file = here("data/processed/ppi_inputs/mm10_ppi.txt"), sep = "\t") #load in ppi data
...
ggplot(cortex_banfi_het, aes(x = cell_type, y = gene_ppi,
  ...
  scale_fill_gradientn(colors = c("#724408", "#8C5209", "white", "#35978F", "#2B766F"),
...
cor_cell <- ggplot(cortex_banfi_trp53, aes(x = cell_type, y = gene_ppi,
  ...
  scale_shape_manual(values = c(Positive = 24, Negative = 25),
                     name = "regNet Score (Shape)") +
  scale_fill_gradientn(colors = c("#8C5209", "white", "#35978F"),
...
apex1_coop_list <- map(apex1_list, ~filter(.x, TF == "Apex1") %>% mutate(cooperator = gene))
trp53_coop_list <- map(trp53_list, ~filter(.x, TF == "Trp53") %>% mutate(cooperator = gene))
mef2c_coop_list <- map(mef2c_list, ~filter(.x, TF == "Mef2c") %>% mutate(cooperator = gene))
parp1_coop_list <- map(parp1_list, ~filter(.x, TF == "Parp1") %>% mutate(cooperator = gene))
e4f1_coop_list <- map(e4f1_list, ~filter(.x, TF == "E4f1") %>% mutate(cooperator = gene))
mzf1_coop_list <- map(mzf1_list, ~filter(.x, TF == "Mzf1") %>% mutate(cooperator = gene))
pbrm1_coop_list <- map(pbrm1_list, ~filter(.x, TF == "Pbrm1") %>% mutate(cooperator = gene))
...
other attached packages:
 [1] plotly_4.10.1         purrr_1.0.1           ggridges_0.5.4        ComplexHeatmap_2.10.0 data.table_1.14.8     ggplot2_3.4.1        
 [7] cowplot_1.1.1         scales_1.2.1          reshape2_1.4.4        stringr_1.5.0         tidyr_1.3.0           RColorBrewer_1.1-3   
[13] styler_1.9.0          readr_2.1.4           dplyr_1.1.0           corrplot_0.92         here_1.0.1            netZooR_1.2.1        
[19] matrixcalc_1.0-6      yarn_1.20.0           pandaR_1.26.0         Biobase_2.54.0        BiocGenerics_0.40.0   reticulate_1.28      
[25] igraph_1.4.1         

loaded via a namespace (and not attached):
  [1] rappdirs_0.3.3              rtracklayer_1.54.0          pbdZMQ_0.3-9                AnnotationForge_1.36.0      R.methodsS3_1.8.2          
  [6] bumphunter_1.36.0           minfi_1.40.0                bit64_4.0.5                 knitr_1.42                  DelayedArray_0.20.0        
 [11] R.utils_2.12.2              KEGGREST_1.34.0             RCurl_1.98-1.10             GEOquery_2.62.2             doParallel_1.0.17          
 [16] generics_0.1.3              GenomicFeatures_1.46.5      preprocessCore_1.56.0       RSQLite_2.3.0               chron_2.3-59               
 [21] bit_4.0.5                   tzdb_0.3.0                  base64url_1.4               xml2_1.3.3                  SummarizedExperiment_1.24.0
 [26] assertthat_0.2.1            STRINGdb_2.6.5              xfun_0.37                   hms_1.1.2                   evaluate_0.20              
 [31] fansi_1.0.4                 restfulr_0.0.15             scrime_1.3.5                progress_1.2.2              caTools_1.18.2             
 [36] dbplyr_2.3.1                htmlwidgets_1.6.1           Rgraphviz_2.38.0            DBI_1.1.3                   reshape_0.8.9              
 [41] stats4_4.1.3                hash_2.2.6.2                ellipsis_0.3.2              backports_1.4.1             permute_0.9-7              
 [46] annotate_1.72.0             biomaRt_2.50.3              sparseMatrixStats_1.6.0     MatrixGenerics_1.6.0        vctrs_0.5.2                
 [51] penalized_0.9-52            cachem_1.0.7                withr_2.5.0                 grr_0.9.5                   vroom_1.6.1                
 [56] vegan_2.6-4                 GenomicAlignments_1.30.0    prettyunits_1.1.1           mclust_6.0.0                cluster_2.1.2              
 [61] lazyeval_0.2.2              IRdisplay_1.1               crayon_1.5.2                uchardet_1.1.1              genefilter_1.76.0          
 [66] labeling_0.4.2              edgeR_3.36.0                pkgconfig_2.0.3             GenomeInfoDb_1.30.1         nlme_3.1-155               
 [71] nnet_7.3-17                 rlang_1.0.6                 RJSONIO_1.3-1.8             lifecycle_1.0.3             downloader_0.4             
 [76] filelock_1.0.2              BiocFileCache_2.2.1         GOstats_2.60.0              quantro_1.28.0              rprojroot_2.0.3            
 [81] matrixStats_0.63.0          graph_1.72.0                rngtools_1.5.2              base64_2.0.1                Matrix_1.5-3               
 [86] IRkernel_1.3.2              Rhdf5lib_1.16.0             base64enc_0.1-3             GlobalOptions_0.1.2         png_0.1-8                  
 [91] viridisLite_0.4.1           rjson_0.2.21                bitops_1.0-7                R.oo_1.25.0                 KernSmooth_2.23-20         
 [96] rhdf5filters_1.6.0          Biostrings_2.62.0           blob_1.2.3                  DelayedMatrixStats_1.16.0   doRNG_1.8.6                
[101] shape_1.4.6                 nor1mix_1.3-0               R.cache_0.16.0              S4Vectors_0.32.4            memoise_2.0.1              
[106] GSEABase_1.56.0             magrittr_2.0.3              plyr_1.8.8                  hexbin_1.28.2               gplots_3.1.3               
[111] zlibbioc_1.40.0             compiler_4.1.3              BiocIO_1.4.0                illuminaio_0.36.0           plotrix_3.8-2              
[116] clue_0.3-64                 Rsamtools_2.10.0            cli_3.6.0                   XVector_0.34.0              Category_2.60.0            
[121] MASS_7.3-55                 mgcv_1.8-39                 tidyselect_1.2.0            stringi_1.7.12              yaml_2.3.7                 
[126] askpass_1.1                 locfit_1.5-9.7              ggrepel_0.9.3               tools_4.1.3                 parallel_4.1.3             
[131] circlize_0.4.15             rstudioapi_0.14             uuid_1.1-0                  foreach_1.5.2               farver_2.1.1               
[136] digest_0.6.31               proto_1.0.0                 quadprog_1.5-8              Rcpp_1.0.10                 GenomicRanges_1.46.1       
[141] siggenes_1.68.0             org.Hs.eg.db_3.14.0         httr_1.4.5                  ggdendro_0.1.23             AnnotationDbi_1.56.2       
[146] colorspace_2.1-0            XML_3.99-0.13               fs_1.6.1                    IRanges_2.28.0              splines_4.1.3              
[151] RBGL_1.70.0                 multtest_2.50.0             xtable_1.8-4                jsonlite_1.8.4              R6_2.5.1                   
[156] RUnit_0.4.32                gsubfn_0.7                  pillar_1.8.1                htmltools_0.5.4             glue_1.6.2                 
[161] fastmap_1.1.1               BiocParallel_1.28.3         beanplot_1.3.1              codetools_0.2-18            utf8_1.2.3                 
[166] lattice_0.20-45             tibble_3.1.8                sqldf_0.4-11                curl_5.0.0                  gtools_3.9.4               
[171] GO.db_3.14.0                openssl_2.0.5               survival_3.3-1              limma_3.50.3                rmarkdown_2.20             
[176] repr_1.1.6                  munsell_0.5.0               GetoptLong_1.0.5            rhdf5_2.38.1                GenomeInfoDbData_1.2.7     
[181] iterators_1.0.14            RCy3_2.14.2                 HDF5Array_1.22.1            gtable_0.3.1               
