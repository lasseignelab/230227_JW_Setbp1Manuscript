...
ggplot(data_plot, aes(x = norm, y = reorder(cell_type, norm))) +
  ...
  scale_fill_manual(values = c("#8C5209", "#35978F"), labels = c("Decreased in S858R", "Increased in S858R")) +
  labs(fill = "Differential Gene Targeting", size = "Number of Genes") +
  scale_size_continuous(breaks = c(1000, 2500, 5000, 10000, 15000)) +
  theme(text = element_text(family = "Helvetica")) +
  theme(
...
data_plot <- read.csv(here("results/diff_targeting/normalized_global_diff_targeting_kidney.csv"))
...
panel_5D <- ggplot(data_plot, aes(x = norm, y = reorder(cell_type, norm))) +
  ...
  scale_size_continuous(breaks = c(1000, 2500, 5000, 7500)) +
  theme(text = element_text(family = "Helvetica")) +
  theme(
    legend.position = "right",
  ) +
  ylab("Kidney Cell Type") +
  xlab("Total Differential Targeting Score") +
  theme(axis.text.x = element_text(color = "black")) +
  theme(axis.title.x = element_text(face = "bold", color = "black")) +
  theme(axis.title.y = element_text(face = "bold", color = "black")) +
...
```{r}
panel_5CD <- (panel_5C | panel_5D)
png()
ggsave(here("results/figures/Fig5CD.png"), dpi = 300, width = 12, height = 4)
panel_5CD
dev.off()
