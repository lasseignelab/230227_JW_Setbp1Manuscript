...
This analysis comes after 'Setbp1_MarkersCortex_07.Rmd' and comes before "Setbp1_DGEcortex_09.Rmd"
...
```{r}
#top 25 cerebral
cerebral_markers_genes %>%
    group_by(cluster) %>%
    top_n(-25, p_val_adj) -> cerebral_top25
cerebral_top25
```
...
VlnPlot(cerebral.int, pyramidal_features, stack = TRUE, sort = FALSE, flip = TRUE) +
...
VlnPlot(cerebral.int, oligo_features, stack = TRUE, sort = FALSE, flip = TRUE) +
...
VlnPlot(cerebral.int, mural_features, stack = TRUE, sort = FALSE, flip = TRUE) +
...
VlnPlot(cerebral.int, endothelial_features, stack = TRUE, sort = FALSE, flip = TRUE) +
...
VlnPlot(cerebral.int, interneurons_features, stack = TRUE, sort = FALSE, flip = TRUE) +
...
VlnPlot(cerebral.int, astro_features, stack = TRUE, sort = FALSE, flip = TRUE) +
...
VlnPlot(cerebral.int, microglia_features, stack = TRUE, sort = FALSE, flip = TRUE) +
...
VlnPlot(cerebral.int, macro_features, stack = TRUE, sort = FALSE, flip = TRUE) +
...
VlnPlot(cerebral.int, ependymal_features, stack = TRUE, sort = FALSE, flip = TRUE) +
...
VlnPlot(cerebral.int, inhibitory_features, stack = TRUE, sort = FALSE, flip = TRUE) +
...
VlnPlot(cerebral.int, excitatory_features, stack = TRUE, sort = FALSE, flip = TRUE) +
...
VlnPlot(cerebral.int, opcs_features, stack = TRUE, sort = FALSE, flip = TRUE) +
...
VlnPlot(cerebral.int, cerebral_features, stack = TRUE, sort = FALSE, flip = TRUE) +
...
DotPlot(cerebral.int, features = cerebral_features) + coord_flip() + RotatedAxis()
dev.off()
```
...
select.resolution = "RNA_snn_res.0.9"
cerebral.int <- SetIdent(cerebral.int, value = select.resolution)
cerebral.int.celltypes <- SetIdent(cerebral.int, value = select.resolution)
cerebral.int.celltypes <- RenameIdents(cerebral.int, 
                                       ...
                                       "31" = "microglia",
                                       "32" = "astrocytes")
cerebral.int.celltypes <- AddMetaData(object = cerebral.int.celltypes, as.vector(cerebral.int.celltypes@active.ident), col.name = "cell_type")
save(cerebral.int.celltypes, file = here("data/setbp1_cerebralintcelltypes.Rdata"))
```
...
VlnPlot(cerebral.int.celltypes, final_cerebral_features, stack = TRUE, sort = FALSE, flip = TRUE) +
        theme(legend.position = "none") + ggtitle("Cerebral Cortex Cell-type Identity Markers")
dev.off()
```
...
```{r}
p1 <- DimPlot(cerebral.int.celltypes, reduction = "umap_harmony", group.by = "type", cols = c("control" = "#644DD4", "heterozygous" = "#5F9EFD"))
p2 <- DimPlot(cerebral.int.celltypes, reduction = "umap_harmony", group.by = "orig.ident")
p3 <- DimPlot(cerebral.int.celltypes, reduction = "umap_harmony", group.by = "cell_type", cols = c("excitatory.neurons" = "#DC71FA", "inhibitory.neurons" = "#00BBDB", "oligodendrocytes" = "#7997FF", "astrocytes" = "#6968B4", "microglia" = "#C216B7", "oligodendrocyte.precursor.cells" = "#00C1AA", "mural.cells" = "#027461"), shuffle = TRUE)
png(filename = here("results/figures/cortex_setbp1_UMAP.png"),
    width = 1500,
    height = 1000)
p1 + p3
dev.off()
...
p1
p2
VlnPlot(cerebral.int.celltypes, final_cerebral_features, stack = TRUE, sort = FALSE , flip = TRUE, assay = "RNA", group.by = select.resolution, fill.by = "ident") +
        theme(legend.position = "none") + ggtitle("Cerebral Cortex Cell-type Identity Markers")
p4 <- VlnPlot(cerebral.int.celltypes, final_cerebral_features, stack = TRUE, sort = FALSE , flip = TRUE, assay = "RNA", group.by = "cell_type", fill.by = "ident") +
        theme(legend.position = "none") + ggtitle("Cerebral Cortex Cell-type Identity Markers")
p5 <- DotPlot(cerebral.int.celltypes, features = final_cerebral_features, group.by = "cell_type") + coord_flip() + RotatedAxis()
p4 + p5
  
split_pt <- SplitObject(cerebral.int.celltypes, split.by = "type")
...
ggplot(ctrl_pt, aes(y = Var1, fill = Var1), ) + geom_bar(aes(x = Freq), stat = "identity") +
  ...
  #scale_fill_manual(values = brewer.pal(12, "Paired")) +
  theme(legend.title = element_blank(), axis.text.x = element_text(angle = 45))
pt <- table(cerebral.int.celltypes@meta.data$cell_type, cerebral.int.celltypes@meta.data$type)
pt <- as.data.frame(pt)
...
pt <- table(Idents(cerebral.int.celltypes), cerebral.int.celltypes$type)
pt <- as.data.frame(pt)
pt$Var1 <- as.character(pt$Var1)
...
```{r}
cerebral.int.celltypes@meta.data %>% 
...
This script is followed by 'Setbp1_DGEcrotex_07.Rmd'