...
save(kidney_seurat_filtered, file = "data/project/lasseigne_lab/JordanWhitlock/test/code_review_check/integration_normalized_JW.Rdata")
```
...
save(top20_kidney, file = "data/project/lasseigne_lab/JordanWhitlock/test/code_review_check/integration_top20_JW.Rdata")
```
...
save(kidney_seurat_filtered, file = "data/project/lasseigne_lab/JordanWhitlock/test/code_review_check/integration_scaled_JW.Rdata")
```
...
```{r PCA-elbow-cortex}
cerebral_seurat_filtered <- RunPCA(cerebral_seurat_filtered, npcs = 50, verbose = FALSE) #has a seed already set in source code
ElbowPlot(cerebral_seurat_filtered, reduction = "pca", ndims = 50)
```

```{r PCA-elbow-kidney}
kidney_seurat_filtered <- RunPCA(kidney_seurat_filtered, npcs = 50, verbose = FALSE) #has a seed set already in source code

save(kidney_seurat_filtered, file = "data/project/lasseigne_lab/JordanWhitlock/test/code_review_check/integration_PCA_JW.Rdata")
...
#### visualize tSNE:
```{r tSNE-cortex}
cerebral_seurat_filtered <- RunTSNE(cerebral_seurat_filtered,
  reduction = "pca",
  dims = 1:30,
  perplexity = 30,
  max_iter = 1000,
...
save(kidney_seurat_filtered, file = "data/project/lasseigne_lab/JordanWhitlock/test/code_review_check/integration_tSNE_JW.Rdata")
...
kidney_seurat_filtered <- RunUMAP(kidney_seurat_filtered,
  ...
  n.epochs = 200, min.dist = 0.3, learning.rate = 1, spread = 1 #note: has seed.use = 42 set as default
)

save(kidney_seurat_filtered, file = "data/project/lasseigne_lab/JordanWhitlock/test/code_review_check/integration_UMAP_JW.Rdata")
...
save(kidney_harmony, file = "data/project/lasseigne_lab/JordanWhitlock/test/code_review_check/integration_harmony_JW.Rdata")
```
...
save(kidney_int, file = "data/project/lasseigne_lab/JordanWhitlock/test/code_review_check/integration_harmonyUMAP_JW.Rdata")
```
...
We find highly variable genes on the object not integrated with harmony.
