...
This analysis comes after 'Setbp1_CellTypesCortex_06.Rmd' and precedes 'Setbp1_PseudobulkCounts10'
...
#### Load in object:
```{r}
load(here("data/results/setbp1_cerebralintcelltypes.Rdata"))
```
...
plot <- FeaturePlot(cerebral.int.celltypes, feature = "Setbp1", reduction = "umap_harmony", split.by = "type", label = TRUE)
plot
#Vln Plot of expression
VlnPlot(object = cerebral.int.celltypes, features = "Setbp1", split.by = "type", assay="RNA")
```

#### split violin plotting of Setbp1 across cell types:
```{r}
# grabbing the Setbp1 expression data and moving from S4 to a dataframe
setbp1_exp <- FetchData(cerebral.int.celltypes, vars = c("Setbp1", "cell_type", "type"))
...
png(filename = here("results/figures/cortex_setbp1_expression.png"),
    width = 1500,
    height = 1000)
ggplot(setbp1_exp, aes(cell_type, Expression, fill = type)) + geom_split_violin() + theme_bw() + scale_fill_manual(values = c("control" = "#644DD4", "heterozygous" = "#5F9EFD")) + ggtitle("Setbp1 Expression Across Cerebral Cortex")
dev.off()
```
...
exneuro <- subset(x = cerebral.int.celltypes, idents = "excitatory.neurons")
...
p <- ggplot(data = excitatory.neurons_het, aes(x = avg_log2FC, y = -log10(p_val))) + geom_point() + theme_minimal()
...
p <- ggplot(data=excitatory.neurons_het, aes(x=avg_log2FC, y=-log10(p_val), col=diffexpressed)) + geom_point() + theme_minimal() 
# Add lines as before...
...
```{r}
# subset seurat object for each cell type 
inneuro <- subset(x = cerebral.int.celltypes, idents = "inhibitory.neurons")
...
p <- ggplot(data = inhibitory.neurons_het, aes(x = avg_log2FC, y = -log10(p_val))) + geom_point() + theme_minimal()
p2 <- p + geom_vline(xintercept = c(-0.1, 0.1), col = "red") + geom_hline(yintercept = -log10(0.05), col = "red")
p2
#add column to data to indicate if DE or not:
...
# Re-plot but this time color the points with "diffexpressed"
p <- ggplot(data=inhibitory.neurons_het, aes(x=avg_log2FC, y=-log10(p_val), col=diffexpressed)) + geom_point() + theme_minimal() 
# Add lines as before...
p2 <- p + geom_vline(xintercept=c(-0.1, 0.1), col="red") +
        geom_hline(yintercept=-log10(0.05), col="red")
p2
...
```{r}
# subset seurat object for each cell type 
astro <- subset(x = cerebral.int.celltypes, idents = "astrocytes")
...
p <- ggplot(data = astrocytes_het, aes(x = avg_log2FC, y = -log10(p_val))) + geom_point() + theme_minimal()
p2 <- p + geom_vline(xintercept = c(-0.1, 0.1), col = "red") + geom_hline(yintercept = -log10(0.05), col = "red")
p2
#add column to data to indicate if DE or not:
...
# Re-plot but this time color the points with "diffexpressed"
p <- ggplot(data=astrocytes_het, aes(x=avg_log2FC, y=-log10(p_val), col=diffexpressed)) + geom_point() + theme_minimal() 
# Add lines as before...
p2 <- p + geom_vline(xintercept=c(-0.1, 0.1), col="red") +
        geom_hline(yintercept=-log10(0.05), col="red")
p2
...
```{r}
# subset seurat object for each cell type 
micro <- subset(x = cerebral.int.celltypes, idents = "microglia")
...
p <- ggplot(data = microglia_het, aes(x = avg_log2FC, y = -log10(p_val))) + geom_point() + theme_minimal()
p2 <- p + geom_vline(xintercept = c(-0.1, 0.1), col = "red") + geom_hline(yintercept = -log10(0.05), col = "red")
p2
#add column to data to indicate if DE or not:
...
# Re-plot but this time color the points with "diffexpressed"
p <- ggplot(data=microglia_het, aes(x=avg_log2FC, y=-log10(p_val), col=diffexpressed)) + geom_point() + theme_minimal() 
# Add lines as before...
p2 <- p + geom_vline(xintercept=c(-0.1, 0.1), col="red") +
        geom_hline(yintercept=-log10(0.05), col="red")
p2
...
```{r}
# subset seurat object for each cell type 
mural <- subset(x = cerebral.int.celltypes, idents = "mural.cells")
...
p <- ggplot(data = mural.cells_het, aes(x = avg_log2FC, y = -log10(p_val))) + geom_point() + theme_minimal()
p2 <- p + geom_vline(xintercept = c(-0.1, 0.1), col = "red") + geom_hline(yintercept = -log10(0.05), col = "red")
p2
#add column to data to indicate if DE or not:
...
# Re-plot but this time color the points with "diffexpressed"
p <- ggplot(data=mural.cells_het, aes(x=avg_log2FC, y=-log10(p_val), col=diffexpressed)) + geom_point() + theme_minimal() 
# Add lines as before...
p2 <- p + geom_vline(xintercept=c(-0.1, 0.1), col="red") +
        geom_hline(yintercept=-log10(0.05), col="red")
p2
...
```{r}
# subset seurat object for each cell type 
opcs <- subset(x = cerebral.int.celltypes, idents = "oligodendrocyte.precursor.cells")
...
p <- ggplot(data = opcs_het, aes(x = avg_log2FC, y = -log10(p_val))) + geom_point() + theme_minimal()
p2 <- p + geom_vline(xintercept = c(-0.1, 0.1), col = "red") + geom_hline(yintercept = -log10(0.05), col = "red")
p2
#add column to data to indicate if DE or not:
...
# Re-plot but this time color the points with "diffexpressed"
p <- ggplot(data=opcs_het, aes(x=avg_log2FC, y=-log10(p_val), col=diffexpressed)) + geom_point() + theme_minimal() 
# Add lines as before...
p2 <- p + geom_vline(xintercept=c(-0.1, 0.1), col="red") +
        geom_hline(yintercept=-log10(0.05), col="red")
p2
...
```{r}
# subset seurat object for each cell type 
oligo <- subset(x = cerebral.int.celltypes, idents = "oligodendrocytes")
...
p <- ggplot(data = oligodendrocytes_het, aes(x = avg_log2FC, y = -log10(p_val))) + geom_point() + theme_minimal()
p2 <- p + geom_vline(xintercept = c(-0.1, 0.1), col = "red") + geom_hline(yintercept = -log10(0.05), col = "red")
p2
#add column to data to indicate if DE or not:
...
# Re-plot but this time color the points with "diffexpressed"
p <- ggplot(data=oligodendrocytes_het, aes(x=avg_log2FC, y=-log10(p_val), col=diffexpressed)) + geom_point() + theme_minimal() 
# Add lines as before...
p2 <- p + geom_vline(xintercept=c(-0.1, 0.1), col="red") +
        geom_hline(yintercept=-log10(0.05), col="red")
p2
...
save(microglia_het, file = here("resullts/DEG/micro_het_DEG01.Rdata"))
```
...
```{r}
ggplot(astrocytes_het, aes(x = avg_log2FC)) + geom_histogram() + ggtitle("Heterozygous -0.1 < Log2FC > 0.1 Distribution in Astro, p-value < 0.05  ")
ggplot(excitatory.neurons_het, aes(x = avg_log2FC)) + geom_histogram() + ggtitle("Heterozygous -0.1 < Log2FC > 0.1 Distribution in Ex neuro")
ggplot(inhibitory.neurons_het, aes(x = avg_log2FC)) + geom_histogram() + ggtitle("Heterozygous -0.1 < Log2FC > 0.1 Distribution in In neuro, p-value < 0.05  ")
ggplot(microglia_het, aes(x = avg_log2FC)) + geom_histogram() + ggtitle("Heterozygous -0.1 < Log2FC > 0.1 Distribution in Micro, p-value < 0.05  ")
ggplot(mural.cells_het, aes(x = avg_log2FC)) + geom_histogram() + ggtitle("Heterozygous -0.1 < Log2FC > 0.1 Distribution in Mural, p-value < 0.05  ")
ggplot(opcs_het, aes(x = avg_log2FC)) + geom_histogram() + ggtitle("Heterozygous -0.1 < Log2FC > 0.1 Distribution in Opcs, p-value < 0.05  ")
ggplot(oligodendrocytes_het, aes(x = avg_log2FC)) + geom_histogram() + ggtitle("Heterozygous -0.1 < Log2FC > 0.1 Distribution in Opcs, p-value < 0.05  ")
```

```{r}
ggplot(oligodendrocytes_data, aes(x = avg_log2FC)) + geom_histogram() + ggtitle("Heterozygous -0.1 < Log2FC > 0.1 Distribution in Oligo, p-value < 0.05  ")
```
...
```{r}
#visualize with FeaturePlot
plot <- FeaturePlot(cerebral.int.celltypes, feature = "Setbp1", reduction = "umap_harmony", split.by = "type")
LabelClusters(plot = plot, id = "ident")

#Vln Plot of expression
VlnPlot(cerebral.int.celltypes, features = "Setbp1", split.by = "type")
```
### Calculating DGE by cell type: log2FC of 0.20
(also doesnt return Setbp1 as a DEG)
---excitatory neurons---
```{r}
# subset seurat object for each cell type 
exneuro <- subset(x = cerebral.int.celltypes, idents = "excitatory.neurons")
Idents(exneuro) <- exneuro@meta.data$type
excitatory.neurons_data<- FindAllMarkers(exneuro, logfc.threshold = 0.20, test.use = "wilcox", only.pos = FALSE, assay = "RNA")
head(excitatory.neurons_data, n = 5)
```
...
excitatory.neurons_het <- excitatory.neurons_data[excitatory.neurons_data$p_val < 0.5 & excitatory.neurons_data$avg_log2FC > 0.20 | excitatory.neurons_data$avg_log2FC < -0.20,]
excitatory.neurons_het <- excitatory.neurons_het[excitatory.neurons_het$cluster == "heterozygous",]
...
excitatory.neurons_het$diffexpressed[excitatory.neurons_het$avg_log2FC > 0.20 & excitatory.neurons_het$p_val < 0.05] <- "UP"
# if log2Foldchange < -0.20 and pvalue < 0.05, set as "DOWN"
excitatory.neurons_het$diffexpressed[excitatory.neurons_het$avg_log2FC < -0.20 & excitatory.neurons_het$p_val < 0.05] <- "DOWN"
# Re-plot but this time color the points with "diffexpressed"
p <- ggplot(data=excitatory.neurons_het, aes(x=avg_log2FC, y=-log10(p_val), col=diffexpressed)) + geom_point() + theme_minimal() 
# Add lines as before...
...
inneuro <- subset(x = cerebral.int.celltypes, idents = "inhibitory.neurons")
Idents(inneuro) <- inneuro@meta.data$type
inhibitory.neurons_data<- FindAllMarkers(inneuro, logfc.threshold = 0.20, test.use = "wilcox", only.pos = FALSE, assay = "RNA")
head(inhibitory.neurons_data, n = 5)
```

```{r}
inhibitory.neurons_het <- inhibitory.neurons_data[inhibitory.neurons_data$p_val < 0.5 & inhibitory.neurons_data$avg_log2FC > 0.20 | inhibitory.neurons_data$avg_log2FC < -0.20,]
inhibitory.neurons_het <- inhibitory.neurons_het[inhibitory.neurons_het$cluster == "heterozygous",]
...
inhibitory.neurons_het$diffexpressed <- "NO"
# if log2Foldchange > 0.20 and pvalue < 0.05, set as "UP" 
inhibitory.neurons_het$diffexpressed[inhibitory.neurons_het$avg_log2FC > 0.20 & inhibitory.neurons_het$p_val < 0.05] <- "UP"
# if log2Foldchange < -0.20 and pvalue < 0.05, set as "DOWN"
inhibitory.neurons_het$diffexpressed[inhibitory.neurons_het$avg_log2FC < -0.20 & inhibitory.neurons_het$p_val < 0.05] <- "DOWN"
# Re-plot but this time color the points with "diffexpressed"
p <- ggplot(data=inhibitory.neurons_het, aes(x=avg_log2FC, y=-log10(p_val), col=diffexpressed)) + geom_point() + theme_minimal() 
# Add lines as before...
p2 <- p + geom_vline(xintercept=c(-0.20, 0.20), col="red") +
...
astro <- subset(x = cerebral.int.celltypes, idents = "astrocytes")
Idents(astro) <- astro@meta.data$type
astrocytes_data<- FindAllMarkers(astro, logfc.threshold = 0.20, test.use = "wilcox", only.pos = FALSE, assay = "RNA")
head(astrocytes_data, n = 5)
```

```{r}
astrocytes_het <- astrocytes_data[astrocytes_data$p_val < 0.5 & astrocytes_data$avg_log2FC > 0.20 | astrocytes_data$avg_log2FC < -0.20,]
astrocytes_het <- astrocytes_het[astrocytes_het$cluster == "heterozygous",]
...
astrocytes_het$diffexpressed <- "NO"
# if log2Foldchange > 0.20 and pvalue < 0.05, set as "UP" 
astrocytes_het$diffexpressed[astrocytes_het$avg_log2FC > 0.20 & astrocytes_het$p_val < 0.05] <- "UP"
# if log2Foldchange < -0.20 and pvalue < 0.05, set as "DOWN"
astrocytes_het$diffexpressed[astrocytes_het$avg_log2FC < -0.20 & astrocytes_het$p_val < 0.05] <- "DOWN"
# Re-plot but this time color the points with "diffexpressed"
p <- ggplot(data=astrocytes_het, aes(x=avg_log2FC, y=-log10(p_val), col=diffexpressed)) + geom_point() + theme_minimal() 
# Add lines as before...
p2 <- p + geom_vline(xintercept=c(-0.20, 0.20), col="red") +
...
micro <- subset(x = cerebral.int.celltypes, idents = "microglia")
Idents(micro) <- micro@meta.data$type
microglia_data<- FindAllMarkers(micro, logfc.threshold = 0.20, test.use = "wilcox", only.pos = FALSE, assay = "RNA")
head(microglia_data, n = 5)
```

```{r}
microglia_het <- microglia_data[microglia_data$p_val < 0.5 & microglia_data$avg_log2FC > 0.20 | microglia_data$avg_log2FC < -0.20,]
microglia_het <- microglia_het[microglia_het$cluster == "heterozygous",]
...
microglia_het$diffexpressed <- "NO"
# if log2Foldchange > 0.20 and pvalue < 0.05, set as "UP" 
microglia_het$diffexpressed[microglia_het$avg_log2FC > 0.20 & microglia_het$p_val < 0.05] <- "UP"
# if log2Foldchange < -0.20 and pvalue < 0.05, set as "DOWN"
microglia_het$diffexpressed[microglia_het$avg_log2FC < -0.20 & microglia_het$p_val < 0.05] <- "DOWN"
# Re-plot but this time color the points with "diffexpressed"
p <- ggplot(data=microglia_het, aes(x=avg_log2FC, y=-log10(p_val), col=diffexpressed)) + geom_point() + theme_minimal() 
# Add lines as before...
p2 <- p + geom_vline(xintercept=c(-0.20, 0.20), col="red") +
...
mural <- subset(x = cerebral.int.celltypes, idents = "mural.cells")
Idents(mural) <- mural@meta.data$type
mural.cells_data<- FindAllMarkers(mural, logfc.threshold = 0.20, test.use = "wilcox", only.pos = FALSE, assay = "RNA")
head(mural.cells_data, n = 5)
```

```{r}
mural.cells_het <- mural.cells_data[mural.cells_data$p_val < 0.5 & mural.cells_data$avg_log2FC > 0.20 | mural.cells_data$avg_log2FC < -0.20,]
mural.cells_het <- mural.cells_het[mural.cells_het$cluster == "heterozygous",]
...
mural.cells_het$diffexpressed <- "NO"
# if log2Foldchange > 0.20 and pvalue < 0.05, set as "UP" 
mural.cells_het$diffexpressed[mural.cells_het$avg_log2FC > 0.20 & mural.cells_het$p_val < 0.05] <- "UP"
# if log2Foldchange < -0.20 and pvalue < 0.05, set as "DOWN"
mural.cells_het$diffexpressed[mural.cells_het$avg_log2FC < -0.20 & mural.cells_het$p_val < 0.05] <- "DOWN"
# Re-plot but this time color the points with "diffexpressed"
p <- ggplot(data=mural.cells_het, aes(x=avg_log2FC, y=-log10(p_val), col=diffexpressed)) + geom_point() + theme_minimal() 
# Add lines as before...
p2 <- p + geom_vline(xintercept=c(-0.20, 0.20), col="red") +
...
opcs <- subset(x = cerebral.int.celltypes, idents = "oligodendrocyte.precursor.cells")
Idents(opcs) <- opcs@meta.data$type
opcs_data<- FindAllMarkers(opcs, logfc.threshold = 0.20, test.use = "wilcox", only.pos = FALSE, assay = "RNA")
head(opcs_data, n = 5)
```

```{r}
opcs_het <- opcs_data[opcs_data$p_val < 0.5 & opcs_data$avg_log2FC > 0.20 | opcs_data$avg_log2FC < -0.20,]
opcs_het <- opcs_het[opcs_het$cluster == "heterozygous",]
...
opcs_het$diffexpressed <- "NO"
# if log2Foldchange > 0.20 and pvalue < 0.05, set as "UP" 
opcs_het$diffexpressed[opcs_het$avg_log2FC > 0.20 & opcs_het$p_val < 0.05] <- "UP"
# if log2Foldchange < -0.20 and pvalue < 0.05, set as "DOWN"
opcs_het$diffexpressed[opcs_het$avg_log2FC < -0.20 & opcs_het$p_val < 0.05] <- "DOWN"
# Re-plot but this time color the points with "diffexpressed"
p <- ggplot(data=opcs_het, aes(x=avg_log2FC, y=-log10(p_val), col=diffexpressed)) + geom_point() + theme_minimal() 
# Add lines as before...
p2 <- p + geom_vline(xintercept=c(-0.20, 0.20), col="red") +
...
oligo <- subset(x = cerebral.int.celltypes, idents = "oligodendrocytes")
Idents(oligo) <- oligo@meta.data$type
oligodendrocytes_data<- FindAllMarkers(oligo, logfc.threshold = 0.20, test.use = "wilcox", only.pos = FALSE, assay = "RNA")
head(oligodendrocytes_data, n = 5)
```

```{r}
oligodendrocytes_het <- oligodendrocytes_data[oligodendrocytes_data$p_val < 0.5 & oligodendrocytes_data$avg_log2FC > 0.20 | oligodendrocytes_data$avg_log2FC < -0.20,]
oligodendrocytes_het <- oligodendrocytes_het[oligodendrocytes_het$cluster == "heterozygous",]
...
oligodendrocytes_het$diffexpressed <- "NO"
# if log2Foldchange > 0.20 and pvalue < 0.05, set as "UP" 
oligodendrocytes_het$diffexpressed[oligodendrocytes_het$avg_log2FC > 0.20 & oligodendrocytes_het$p_val < 0.05] <- "UP"
# if log2Foldchange < -0.20 and pvalue < 0.05, set as "DOWN"
oligodendrocytes_het$diffexpressed[oligodendrocytes_het$avg_log2FC < -0.20 & oligodendrocytes_het$p_val < 0.05] <- "DOWN"
# Re-plot but this time color the points with "diffexpressed"
p <- ggplot(data=oligodendrocytes_het, aes(x=avg_log2FC, y=-log10(p_val), col=diffexpressed)) + geom_point() + theme_minimal() 
# Add lines as before...
p2 <- p + geom_vline(xintercept=c(-0.20, 0.20), col="red") +
...
### For loop to generate DEG output at log2FC threshold of 0.1 (replaces all code above, except plotting, need to add that to loop later):
```{r}
#specify the cell types in the data to loop through
celltypes <- levels(cerebral.int.celltypes)

#create empty dataframe to store the loop outputs within
DEGs_cerebralcells <- data.frame()

#for loop, loops through each of the 'celltypes' in cerebral and calulates the DEG between conditions (heterozygous and control)
for(i in celltypes){
  # subset seurat data for each cerebral cell type ----------
  celltype <- subset(cerebral.int.celltypes, idents = i)
...
#write.csv(DEGs_cerebralcells, file = "/data/user/jbarham3/jw_cellspecific/jw_cellspecific/setbp1_clean/outputs/DEGs_cerebralcells.csv") #commented out after run once
...
#generating seurat input for complex heatmap from normalized expression data
resource: 
* NormalizeData, ScaleData  was not used because of https://www.embopress.org/doi/full/10.15252/msb.20188746

```{r}
DefaultAssay(cerebral.int.celltypes) #RNA

#filter for just setbp1 targets
cerebral.subset <- subset(cerebral.int.celltypes, features = setbp1_genes)

#grab meta data for plot:
meta.data <- cerebral.subset@meta.data
...
mat<- GetAssayData(cerebral.subset, assay = "RNA", slot = "data")
#genes_mat <- subset(mat, rownames(mat) %in% setbp1_genes)
...
# png(filename = "cortex_expression_heatmap.png",
#     width = 1000,
#     height = 1000)
Heatmap(
  genes_mat,
...
cerebral.subset <- DEGs_cerebralcells[DEGs_cerebralcells$gene %in% setbp1_genes,]
#rownames(cerebral.subset) <- cerebral.subset$X

#grab just log2fc values from DEG matrix, keep rownames and then order (below) by rowname
genes_mat <- cerebral.subset[, c("avg_log2FC", "cell_type", "gene", "cluster"), drop = FALSE]
...
annotation_colors <- list("condition" = c("control" = "#644DD4", "heterozygous" = "#5F9EFD"), "cell_type" = c("excitatory.neurons" = "#DC71FA", "inhibitory.neurons" = "#00BBDB", "oligodendrocytes" = "#7997FF", "astrocytes" = "#6968B4", "microglia" = "#C216B7", "oligodendrocyte.precursor.cells" = "#00C1AA", "mural.cells" = "#027461"))
...
png(filename = here("results/figures/cortex_DEGsetbp1_heatmap.png"),
    width = 1000,
    height = 1000)
Heatmap(mat,
  col = col_fun,
