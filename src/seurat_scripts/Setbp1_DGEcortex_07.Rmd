...
save(fibroblasts_het, file = here("resullts/DEG/fibro_het_DEG01.Rdata"))
```
...
```{r}
# visualize with FeaturePlot
plot <- FeaturePlot(cerebral_int_celltypes, feature = "Setbp1", reduction = "umap_harmony", split.by = "type")
LabelClusters(plot = plot, id = "ident")

# Vln Plot of expression
VlnPlot(cerebral_int_celltypes, features = "Setbp1", split.by = "type")
```
...
```{r}
# subset seurat object for each cell type
fibroblasts <- subset(x = cerebral_int_celltypes, idents = "fibroblasts")
Idents(fibroblasts) <- fibroblasts@meta.data$type
excitatory.neurons_data <- FindAllMarkers(fibroblasts, logfc.threshold = 0.20, test.use = "wilcox", only.pos = FALSE, assay = "RNA")
head(fibroblasts_data, n = 5)
```

```{r}
# plotting just the DEG from Het:
fibroblasts_het <- fibroblasts_data[fibroblasts_data$p_val < 0.5 & fibroblasts_data$avg_log2FC > 0.20 | fibroblasts_data$avg_log2FC < -0.20, ]
fibroblasts_het <- fibroblasts_het[fibroblasts_het$cluster == "heterozygous", ]
...
fibroblasts_het$diffexpressed <- "NO"
# if log2Foldchange > 0.20 and pvalue < 0.05, set as "UP"
fibroblasts_het$diffexpressed[fibroblasts_het$avg_log2FC > 0.20 & fibroblasts_het$p_val < 0.05] <- "UP"
# if log2Foldchange < -0.20 and pvalue < 0.05, set as "DOWN"
fibroblasts_het$diffexpressed[fibroblasts_het$avg_log2FC < -0.20 & fibroblasts_het$p_val < 0.05] <- "DOWN"
# Re-plot but this time color the points with "diffexpressed"
p <- ggplot(data = fibroblasts_het, aes(x = avg_log2FC, y = -log10(p_val), col = diffexpressed)) +
  geom_point() +
  theme_minimal()
...
```{r}
DefaultAssay(cerebral_int_celltypes) # RNA

# filter for just setbp1 targets
cerebral_subset <- subset(cerebral_int_celltypes, features = setbp1_genes)

# grab meta data for plot:
meta.data <- cerebral_subset@meta.data
...
mat <- GetAssayData(cerebral_subset, assay = "RNA", slot = "data")
