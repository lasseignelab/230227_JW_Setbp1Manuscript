...
```{r}
excitatory_neurons_het <- excitatory_neurons_data[excitatory_neurons_data$p_val_adj < 0.5 & excitatory_neurons_data$avg_log2FC > 0.1 | excitatory_neurons_data$avg_log2FC < -0.1, ]
...
```{r}
inhibitory_neurons_het <- inhibitory_neurons_data[inhibitory_neurons_data$p_val_adj < 0.5 & inhibitory_neurons_data$avg_log2FC > 0.1 | inhibitory_neurons_data$avg_log2FC < -0.1, ]
...
```{r}
astrocytes_het <- astrocytes_data[astrocytes_data$p_val_adj < 0.5 & astrocytes_data$avg_log2FC > 0.1 | astrocytes_data$avg_log2FC < -0.1, ]
...
```{r}
microglia_het <- microglia_data[microglia_data$p_val_adj < 0.5 & microglia_data$avg_log2FC > 0.1 | microglia_data$avg_log2FC < -0.1, ]
...
```{r}
# subset seurat object for each cell type
mural <- subset(x = cerebral_int_celltypes, idents = "mural_cells")
Idents(mural) <- mural@meta.data$type
mural_cells_data <- FindAllMarkers(mural, logfc.threshold = 0.1, test.use = "wilcox", only.pos = FALSE, assay = "RNA")
head(mural_cells_data, n = 5)
```

```{r}
mural_cells_het <- mural_cells_data[mural_cells_data$p_val_adj < 0.5 & mural_cells_data$avg_log2FC > 0.1 | mural_cells_data$avg_log2FC < -0.1, ]
mural_cells_het <- mural_cells_het[mural_cells_het$cluster == "heterozygous", ]

# add column to data to indicate if DE or not:
mural_cells_het$diffexpressed <- "NO"
# if log2Foldchange > 0.1 and pvalue < 0.05, set as "UP"
mural_cells_het$diffexpressed[mural_cells_het$avg_log2FC > 0.1 & mural_cells_het$p_val_adj < 0.05] <- "UP"
# if log2Foldchange < -0.1 and pvalue < 0.05, set as "DOWN"
mural_cells_het$diffexpressed[mural_cells_het$avg_log2FC < -0.1 & mural_cells_het$p_val_adj < 0.05] <- "DOWN"
```
...
```{r}
opcs_het <- opcs_data[opcs_data$p_val_adj < 0.5 & opcs_data$avg_log2FC > 0.1 | opcs_data$avg_log2FC < -0.1, ]
...
```{r}
oligodendrocytes_het <- oligodendrocytes_data[oligodendrocytes_data$p_val_adj < 0.5 & oligodendrocytes_data$avg_log2FC > 0.1 | oligodendrocytes_data$avg_log2FC < -0.1, ]
...
```{r}
fibroblasts_het <- fibroblasts_data[fibroblasts_data$p_val_adj < 0.5 & fibroblasts_data$avg_log2FC > 0.1 | fibroblasts_data$avg_log2FC < -0.1, ]
...
write.csv(mural_cells_het, here("results/seurat/DGE_06/DEG_mural_cells.csv"), row.names = FALSE)
...
save(mural_cells_het, file = here("results/DEG/mural_het_DEG01.Rdata"))
