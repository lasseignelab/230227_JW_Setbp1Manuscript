...
```{r}
vcam1_exp <- FetchData(kidney_int_celltypes, vars = c("Vcam1", "cell_type", "type"))
colnames(setbp1_exp) <- c("expression", "cell type", "condition")

ggplot(vcam1_exp, aes(`cell type`, expression, fill = condition)) +
  geom_split_violin() +
  theme_bw() +
...
png(filename = here("results/figures/SUPP_Vcam1.png"),
    width = 1200,
    height = 1000,
    res = 250)
VlnPlot(kidney_int_celltypes, features = "Vcam1", split.by = "type", pt.size = 0) +
  theme_bw() +
  scale_fill_manual(values = c("control" = "#FE6100", "heterozygous" = "#FFB000")) + 
  theme(text = element_text(family = "Helvetica")) +
  theme(axis.text.x = element_text(angle = 45, color = "black", hjust = 1)) + 
...
FeaturePlot(kidney_int_celltypes, features = "Vcam1", split.by = "type", reduction = "umap_harmony", label = TRUE)
```

```{r}
kidney_repair <- c("Slc7a13", "Slc5a12", "Slc22a30", "Sema5a", "Havcr1", "Dcdc2a", "Myc", "Krt20", "Sox9")

for(i in kidney_repair){
png(filename = here(paste0("results/figures/SUPP_", i, ".png")),
    width = 1500,
    height = 1000,
    res = 250)
...
```{r}
style_file(here("src/module_scores/16_failed_repair_kidney_profiling.Rmd")) # Note: lintr was run as well
```
