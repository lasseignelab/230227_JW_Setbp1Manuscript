...
write.csv(het_cortex_DEGS, file = here("results/seurat/Pathways_08/het_cortex_DEG.csv"))
```
...
# saving dataframe as a csv to outputs folder
write.csv(het_kidney_DEGS, file = here("results/seurat/Pathways_08/het_kidney_DEG.csv"))
```
...
for (i in cell_types) {
  ...
  png(paste0(here("results/seurat/Pathways_08/"), "FEA_pathways_up_", i, ".png"),
...
save(cortex_UP_result_list, file = here("results/seurat/Pathways_08/cortex_het_fea_DGE_UP.Rdata"))
...
for (i in cell_types) {
  ...
  # set png to save plot to ----------
  png(paste0(here("results/seurat/Pathways_08/"), "FEA_pathways_down_",i, ".png"),
      width = 1000, height = 1000)
  # plot dotplot and add title based on input variable ----------
  plot <- bubbleplot(fea_result_filt) +
...
save(cortex_DOWN_result_list, file = here("results/seurat/Pathways_08/cortex_het_fea_DGE_DOWN.Rdata"))
```
...
save(kidney_UP_result_list, file = here("results/seurat/Pathways_08/kidney_het_fea_DGE_UP.Rdata"))
...
save(kidney_DOWN_result_list, file = here("results/seurat/Pathways_08/kidney_het_fea_DGE_DOWN.Rdata"))
```
...
for (i in cell_types) {
  ...
  # plot dotplot and add title based on input variable ----------
    png(paste0(here("results/seurat/Pathways_08/"), "FEA_SETBP1targetspathways_kidney_", i, ".png"),
...
for (i in cell_types) {
  ...
  if(nrow(targets)>0){
    fea_res <- combined_fea_set(genes = targets, organism = "mmusculus")
  # set png to save plot to ----------
  png(paste0(here("results/seurat/Pathways_08/"), "FEA_Setbp1targetspathways_cortex_", i, ".png"),
      width = 1000, height = 3000)
  # plot dotplot and add title based on input variable ----------
  plot <- ggplot(fea_res$combined, aes(x = direction,
                                       y = reorder(term_name, -p_value),
...
```
Note: [1] "no DE targets in mural.cells"
