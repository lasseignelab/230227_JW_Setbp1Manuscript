...
bcell_het <- bcell_data[bcell_data$p_val < 0.5 &
...
```{r}
# subsetting just the DEG from Het:
CDIC_typeA_het <- CDIC_typeA_data[CDIC_typeA_data$p_val < 0.5 &
...
CDIC_typeA_het$diffexpressed[CDIC_typeA_het$avg_log2FC > 0.1 & 
                           CDIC_typeA_het$p_val < 0.05] <- "UP"
# if log2Foldchange < -0.1 and pvalue < 0.05, set as "DOWN"
CDIC_typeA_het$diffexpressed[CDIC_typeA_het$avg_log2FC < -0.1 & 
                           CDIC_typeA_het$p_val < 0.05] <- "DOWN"
```
...
```{r}
CDIC_typeB_het <- CDIC_typeB_data[CDIC_typeB_data$p_val < 0.5 &
...
CDIC_typeB_het$diffexpressed[CDIC_typeB_het$avg_log2FC > 0.1 & 
                           CDIC_typeB_het$p_val < 0.05] <- "UP"
# if log2Foldchange < -0.1 and pvalue < 0.05, set as "DOWN"
CDIC_typeB_het$diffexpressed[CDIC_typeB_het$avg_log2FC < -0.1 & 
                           CDIC_typeB_het$p_val < 0.05] <- "DOWN"
```
...
```{r}
CDPC_het <- CDPC_data[CDPC_data$p_val < 0.5 &
...
CDPC_het$diffexpressed[CDPC_het$avg_log2FC > 0.1 & 
                          CDPC_het$p_val < 0.05] <- "UP"
# if log2Foldchange < -0.1 and pvalue < 0.05, set as "DOWN"
CDPC_het$diffexpressed[CDPC_het$avg_log2FC < -0.1 & 
                          CDPC_het$p_val < 0.05] <- "DOWN"
```
...
```{r}
# subsetting just the DEG from Het:
PCTS1_het <- PCTS1_data[PCTS1_data$p_val < 0.5 &
...
PCTS1_het$diffexpressed[PCTS1_het$avg_log2FC > 0.1 & 
                           PCTS1_het$p_val < 0.05] <- "UP"
# if log2Foldchange < -0.1 and pvalue < 0.05, set as "DOWN"
PCTS1_het$diffexpressed[PCTS1_het$avg_log2FC < -0.1 & 
                           PCTS1_het$p_val < 0.05] <- "DOWN"
```
...
```{r}
# subsetting just the DEG from Het:
PCTS2_het <- PCTS2_data[PCTS2_data$p_val < 0.5 &
...
PCTS2_het$diffexpressed[PCTS2_het$avg_log2FC > 0.1 & 
                              PCTS2_het$p_val < 0.05] <- "UP"
# if log2Foldchange < -0.1 and pvalue < 0.05, set as "DOWN"
PCTS2_het$diffexpressed[PCTS2_het$avg_log2FC < -0.1 & 
                              PCTS2_het$p_val < 0.05] <- "DOWN"
```
...
```{r}
# subsetting just the DEG from Het:
smcs_het <- smcs_data[smcs_data$p_val < 0.5 &
...
smcs_het$diffexpressed[smcs_het$avg_log2FC > 0.1 & 
                              smcs_het$p_val < 0.05] <- "UP"
# if log2Foldchange < -0.1 and pvalue < 0.05, set as "DOWN"
smcs_het$diffexpressed[smcs_het$avg_log2FC < -0.1 & 
                              smcs_het$p_val < 0.05] <- "DOWN"
```
...
```{r}
# subsetting just the DEG from Het:
prox_tubule_het <- prox_tubule_data[prox_tubule_data$p_val < 0.5 &
...
prox_tubule_het$diffexpressed[prox_tubule_het$avg_log2FC > 0.1 & 
                              prox_tubule_het$p_val < 0.05] <- "UP"
# if log2Foldchange < -0.1 and pvalue < 0.05, set as "DOWN"
prox_tubule_het$diffexpressed[prox_tubule_het$avg_log2FC < -0.1 & 
                              prox_tubule_het$p_val < 0.05] <- "DOWN"
```

Outputs at log2FC of 0.1:
```{r}
save(bcell_het, file = here("results/DEG/bcell_het_DEG01.Rdata"))
save(CDPC_het, file = here("results/DEG/CDPC_het_DEG01.Rdata"))
save(CDIC_typeA_het, file = here("results/DEG/CDICA_het_DEG01.Rdata"))
save(CDIC_typeB_het, file = here("results/DEG/CDICB_het_DEG01.Rdata"))
...
save(PCTS1_het, file = here("results/DEG/PCTS1_het_DEG01.Rdata"))
...
save(PCTS2_het, file = here("results/DEG/PCTS2_het_DEG01.Rdata"))
save(smcs_het, file = here("results/DEG/smcs_het_DEG01.Rdata"))
save(prox_tubule_het, file = here("results/DEG/prox_tubule_het_DEG01.Rdata"))
```
